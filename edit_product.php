...
<main>
  ...
  <div class="flex justify-center align-middle">
    ...
    <form id="loginForm" method="post" class="flex flex-col align-middle gap-4 mb-10" novalidate>
      ...
      <?php endif; ?>
      <input type="text" name="name" value="<?php echo $_POST["name"] ?? $contact["name"] ?? "" ?>" placeholder="Product Name">
      <?php if (isset($errors['price'])) : ?>
        <p class="error text-center"><?= $errors['price'] ?></p>
      <?php endif; ?>
      <input type="number" name="price" value="<?php echo $_POST["price"] ?? $contact["price"] ?? "" ?>" placeholder="price">
      <?php if (isset($errors['category'])) : ?>
        <p class="error text-center"><?= $errors['category'] ?></p>
      <?php endif; ?>
      <input type="text" name="category" value="<?php echo $_POST["category"] ?? $contact["category"] ?? "" ?>" placeholder="category">
      <?php if (isset($errors['stock'])) : ?>
        <p class="error text-center"><?= $errors['stock'] ?></p>
      <?php endif; ?>
      <input type="number" name="stock" value="<?php echo $_POST["stock"] ?? $contact["stock"] ?? "" ?>" placeholder="stock">
      <?php if (isset($errors['image'])) : ?>
        <p class="error text-center"><?= $errors['image'] ?></p>
      <?php endif; ?>
      <input type="url" name="image" value="<?php echo $_POST["image"] ?? $contact["image"] ?? "" ?>" placeholder="image url">
      <?php if (isset($errors['description'])) : ?>
        <p class="error text-center"><?= $errors['description'] ?></p>
      <?php endif; ?>
      <textarea name="description" value="<?php echo $_POST["description"] ?? $contact["description"] ?? "" ?>" placeholder="description"></textarea>
