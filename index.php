...
if (count($_GET) > 0) {
  ...
  $minPrice = isset($_GET["minPrice"]) ? (int)trim($_GET["minPrice"]) : null;
  $maxPrice = isset($_GET["maxPrice"]) ? (int)trim($_GET["maxPrice"]) : null;
  if ($category !== '') {
        ...
        <div class="card">
          ...
          <div class="card-btns">
            ...
            <?php if ((int)$product["stock"] > 0) : ?>
              <a href="add_to_cart.php?id=<?= $product["id"] ?>" class="add-btn btn bg-gray-700">Add to Cart</a>
            <?php else : ?>
